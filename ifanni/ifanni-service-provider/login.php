...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Sanitize user inputs to prevent SQL injection
     ...
     $password = mysqli_real_escape_string($db, $_POST['service_provider_password']);




    //$row = DB::queryFirstRow("select * from service_provider WHERE service_provider_email = '$email' AND service_provider_password = '$password' ");
 
    //if(DB::count() == 1)
    //{
     // $_SESSION['service-provider_id'] = $row['id'];
     // $_SESSION['login_user'] = $email;

      // Set the service Provider name session variable
      //$_SESSION['service_provider_name'] = $row['service_Provider_name'];
  //  }
 ...
 // }
    ...
    $sql = "SELECT * FROM service_provider WHERE service_provider_email = '$email' AND service_provider_password = '$password'";
    ...
    if ($count == 1) {
        ...
        $_SESSION['login_user'] = $email;
         ...
         if ($remember_me) {
          ...
          $cookie_expire = time() + 30 * 24 * 3600; // 30 days (adjust as needed)

          // Set the cookie
          setcookie($cookie_name, $cookie_value, $cookie_expire, "/"); // "/" means the whole domain

          // You can also set a "remember_me_token" in the database for additional security
          // Make sure to hash the token before storing it in the database
          // Store the token in the user's record in the database and update it when needed
      }
        ...
        // Display an error message if login fails
        $error = $count."-- Your Login Name or Password is invalid";
    }
  }
?>
<!DOCTYPE html>
